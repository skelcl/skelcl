...
Map<void(Index)>::Map(const Source& source, const std::string& funcName)
  : Skeleton(),
...
__kernel void SCL_MAP(const unsigned int SCL_ELEMENTS,
...
// ## Map<IndexPoint, void> ################################################
Map<void(IndexPoint)>::Map(const Source& source, const std::string& funcName)
  : Skeleton(),
    detail::MapHelper<void(IndexPoint)>(createAndBuildProgram(source, funcName))
{
}

detail::Program Map<void(IndexPoint)>::createAndBuildProgram(
...
__kernel void SCL_MAP(const unsigned int SCL_COL_COUNT,
                      const unsigned int SCL_ROW_COUNT,
                      const unsigned int SCL_ROW_OFFSET)
{
  if ( get_global_id(1) < SCL_COL_COUNT && get_global_id(0) < SCL_ROW_COUNT ) {
