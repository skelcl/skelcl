...
int main(int argc, char** argv) {
	...
	skelcl::MapOverlap<float(float)> m(std::ifstream { "./cannyGauss.cl" },
			static_cast<unsigned int>(range), detail::Padding::NEUTRAL, 255,
			"func");
	...
	skelcl::MapOverlap<float(float)> p(std::ifstream { "./cannyThreshold.cl" },
			1, detail::Padding::NEAREST, 255, "func");

	time2 = get_time();

	Matrix<float> outputImage = m(inputImage, kernelVec,
