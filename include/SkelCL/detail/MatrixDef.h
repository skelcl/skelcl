...
template<typename T>
typename Matrix<T>::iterator Matrix<T>::row_end(
		typename coordinate::index_type rowIndex) {
	return row_begin(rowIndex + 1);
}

template<typename T>
typename Matrix<T>::const_iterator Matrix<T>::row_end(
		typename coordinate::index_type rowIndex) const {
	return row_begin(rowIndex + 1);
}

template<typename T>
typename Matrix<T>::iterator Matrix<T>::operator[](
		typename coordinate::index_type rowIndex) {
	return row_begin(rowIndex);
}

template<typename T>
typename Matrix<T>::const_iterator Matrix<T>::operator[](
		typename coordinate::index_type rowIndex) const {
	return row_begin(rowIndex);
}
...
bool Matrix<T>::empty() const {
	return (_size.rowCount() == 0 || _size.columnCount() == 0);
}
...
T Matrix<T>::operator()(int row, int col) {
	return _hostBuffer[row * _size.columnCount() + col];
}

template<typename T>
typename Matrix<T>::reference Matrix<T>::at(coordinate c) {
	// check for out of bound access
	if (!(c.rowIndex < _size.rowCount() && c.columnIndex < _size.columnCount())) {
		std::stringstream errSS;
		errSS << "Given index (" << c.rowIndex << ", " << c.columnIndex
				<< ") for access is out of bound. " << "Boundaries are ("
...
template<typename T>
typename Matrix<T>::iterator Matrix<T>::erase_row(
		typename coordinate::index_type rowIndex) {
	_size = {_size.rowCount() -1 , _size.columnCount()};
...
detail::Event Matrix<T>::startUpload() const {
	...
	LOG_DEBUG("Try to Upload a Matrix of Size ", _size.elemCount());
...
//template<typename T>
//const detail::DeviceBuffer&
//Matrix<T>::deviceBuffer(const std::shared_ptr<skelcl::detail::Device>& device) const {
//	return _deviceBuffers[device->id()];
//}
